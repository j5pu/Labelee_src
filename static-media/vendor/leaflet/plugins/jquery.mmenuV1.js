...
//			setLocationHref		: null,
	...
	$.fn.mmenu.search = function( $m, opts )
	{
		...
		if ( opts.add )
		{
			...
			$s.append( '<input placeholder="' + opts.placeholder + '" type="text" autocomplete="off" /><a class="search-cancel" href="#" title="Cancel"><button>X</button></a>' );
							...
							function()
							{
								...
								if ( $i.length )
								{
                                    if ($p)
                                    {
                                        ...
                                        if ($i.length != $t.find('li').length)
                                        {
                                            $p.addClass(_c.opened);
                                        }

                                        if ($('input[type=text]').val().length == 0)
                                        {
                                            $p.removeClass(_c.opened);
                                        }
